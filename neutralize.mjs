...
async function main() {
  ...
  if (!titles.length) { writeFileSync('out-headlines.json', JSON.stringify({ headlines: {} }, null, 2)); console.error('対象見出しなし。'); return; }
  let ja = [];
  for (let t = 0; t < 6; t++) {
    try { ja = await neutralizeBatch(titles); break; }
    catch (e) {
      if (t === 5) { console.error('  中立見出し生成失敗（無料枠の混雑/枯渇？）: ' + e.message); break; }
      await new Promise(r => setTimeout(r, Math.min(500 * 2 ** t, 8000)));
    }
  }
  // 行数不一致＝対応関係が保証できない→全破棄（別見出しの中立版が付く事故を防ぐ。translate.mjsと同じ規律）
  if (ja.length !== titles.length) {
    if (ja.length) console.error('  行数不一致（入力' + titles.length + '/出力' + ja.length + '）→ 全破棄。原見出しのまま表示されます。');
    ja = [];
  }
  const headlines = {};
  titles.forEach((t, i) => { if (ja[i]) headlines[t] = ja[i]; });
  // 枯渇/失敗で0件になった時、既にある良いデータを空で上書きしない（前回データを温存）
  if (!Object.keys(headlines).length) {
    try { const old = JSON.parse(readFileSync('out-headlines.json','utf8')); if (Object.keys(old.headlines||{}).length) { console.error('  0件→前回の中立見出しを温存'); return; } } catch(e){}
  }
  ...
  console.error('中立見出し ' + Object.keys(headlines).length + '/' + titles.length + ' 件 → out-headlines.json');
